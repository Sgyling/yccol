...
################################################################################
"""
def accuracy_1(file_path):
    try:
        print()
        # 设置验证次数
        yanzhengcishu = 'error:未知'
        proxies = None
        # 读取文件
        # file_path = 'output/diqu/cn.txt'
        # 验证信息
...
def accuracy_1_http():
    ...
    try:
        with open('output/diqu/cn.txt', 'r') as file:
            # 读取所有行并去掉空行
...
def accuracy_4(file_path):
    try:
        ...
        print('---------------------------------------------------------')
        # 读取并且返回
        proxies = accuracy_4_http(file_path)
        if proxies is None:
