...
from get_response.http_yanzheng import read_proxies_from_file
...
def main_qiehuan(input_qiehuan):
    try:
        ...
        # with open('output/http.txt', 'r') as file:
            proxies = read_proxies_from_file(input_qiehuan)
        if not proxies:
            # print("代理文件为空，请检查 http.txt 文件。")
            return "代理为空,请检查上面的那个框框里面有没有代理"
        index = 0
        while True:
            proxy = proxies[index]
            set_proxy(proxy)
