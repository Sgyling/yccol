...
# 从文件中读取代理列表
def read_proxies_from_file(filename):
    # 假设 text_data 是每行一个代理的字符串
    proxies = filename.strip().split('\n')
...
def main_yanzheng(input):
    ...
    # 读取代理列表
    proxies = read_proxies_from_file(input)
    if not proxies:
        # print(f"文件 '{input}' 中没有找到有效的代理。")
        return "没有找到有效的代理。"

    print(f"读取到 {len(proxies)} 个代理，开始测试...")
    ...
    #     print(working_proxies, output_filename)
