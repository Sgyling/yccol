...
def process_http_fofa(html):
    ...
    for ip_data in ip_list:
        ...
        try:
            ...
            print(f"请求 {ip_data} 时发生网络请求错误: {req_err}，跳过该 IP，继续处理下一个。")
        except json.JSONDecodeError as json_err:
            print(f"解析 {ip_data} 返回的 JSON 数据时出错: {json_err}，跳过该 IP，继续处理下一个。")
        except KeyError as key_err:
            print(f"在处理 {ip_data} 返回的数据时，未找到所需的键: {key_err}，跳过该 IP，继续处理下一个。")
        except Exception as e:
            import traceback
            print(f"处理 {ip_data} 时发生未知错误: {e}，详细堆栈信息如下：")
            traceback.print_exc()
