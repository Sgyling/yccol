...
def save_proxies(proxies):
    """保存代理列表到文件"""
    output_dir = "output"
    output_path = os.path.join(output_dir, "socks5_v.txt")
    ...
    a =f"""
    -----------------------------------------------
    # 成功保存 {len(proxies)} 个代理到 {output_path}  #
    -----------------------------------------------
    """
    print(a)
...
def main():
    try:
        ...
        # 2. 构造请求URL
        url = f"http://localhost:5000/api/key?key={key}&mac_id={mac_id}&proxy=socks5&area=all"
